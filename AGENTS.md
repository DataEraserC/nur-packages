...
- **Rust 编译期 rustc 栈溢出（`EffectiveVisibilitiesVisitor::update_decl_chain`）**：nixpkgs 锁定的 rustc 若比上游 CI 用的新，编译大型 Rust 应用（尤其含大量 `pub(crate) use 子模块::*;` 且子模块 `use super::*;` 形成 glob 循环的 Tauri 应用，如 EasyCLIProxyAPI）会报 `rustc unexpectedly overflowed its stack`，`addr2line` 定位到 `rustc_resolve::effective_visibilities::EffectiveVisibilitiesVisitor::update_decl_chain`，并打印 `### cycle encountered after N frames with period M`。这是 rustc 回归（上游工作流锁定旧版，如 `dtolnay/rust-toolchain@1.97.0`，同源码在 1.97 下可编），与 `RUST_MIN_STACK`/`-C opt-level` 无关，单纯加大栈只是拖延（提示值随设置翻倍），源码级只能拆分 glob 循环，代价过大。仓库锁定的 nixpkgs 通常不含旧版 rustc（`rust_1_98` 是唯一版本，`rust-bin`/`rust-overlay` 也不在作用域）。可行出路：改打上游预编译 release（`fetchurl` 指向 GitHub release 资源 + `nix-update-script { }` 可自动识别资源 URL 并更新），并设 `sourceProvenance = [ binaryNativeCode ]`。不要为单个包往 flake 加 nixpkgs/rust-overlay 输入——fork 迁移时 flake.nix 会被上游覆盖，输入不保留；包内 `builtins.fetchTarball` 拉 rust-overlay 虽可行但引入求值期网络与版本兼容风险，非必要不用。
- **Tauri 应用从源码构建必须启用 `custom-protocol` feature**：`tauri` crate 的 build.rs 用 `cargo:dev=!custom-protocol` 决定是否启用 `--cfg dev`；`buildRustPackage` 不会自动加该 feature，默认得到 `dev=true`，应用会去连 `devUrl`（localhost:1420）而不是内嵌 `frontendDist` 的静态资源，产出的包不可用。应设 `cargoBuildFlags = [ "--features" "tauri/custom-protocol" ]`（cargo 支持 `dep/feature` 语法，无需应用自身声明该 feature）；构建前还要把前端产物放到 `tauri.conf.json` 的 `frontendDist` 指向的目录（如仓库根 `dist/`）。可用 `nix log` 里 rustc 命令是否仍带 `--cfg dev` 验证。
- **`wrapGAppsHook3` 会包装 `$out` 下所有可执行文件（含 `libexec/`）**：它不限于 `$out/bin`，会把 `libexec` 里的真实 GUI 二进制也改名为 `.X-wrapped` 并在原位放一个 exec 绝对 store 路径的 wrapper。若再让 `$out/bin` 的启动脚本去 `cp` 该二进制，拷到的是“exec store 里 .X-wrapped”的 wrapper，进程 `current_exe()` 仍落在只读 store，所有“以可执行文件所在目录为可写基准目录”的假设都会失效。修复：把真实二进制以 **0644**（去掉可执行位）装进 `$out/libexec/<pkg>/`，`wrapGAppsHook3` 的 `find -executable` 会跳过它（`autoPatchelfHook` 仍会按 ELF magic 正常修补），启动脚本再用 `install -m755` 从 store 复制到可写目录。
- **应用把 `current_exe().parent()` 当可写数据目录且 store 只读**：部分桌面应用（Tauri/便携版）在 Linux 上把配置、缓存、内核/资源目录统统放在可执行文件旁（`core_base_dir()` 只在 macOS 回退到 `Application Support`，Linux 直接返回 `executable_dir()`），Nix store 只读会立即失败，且预编译二进制无法打补丁。修复：`$out/bin/<main>` 写成启动脚本，每次启动把二进制与随附资源 `install -C`（coreutils，仅内容变化时复制）到 `${XDG_DATA_HOME:-$HOME/.local/share}/<pkg>` 并 `exec` 处的副本，使 `current_exe()` 落在可写目录；同时**不要**安装上游的便携版清单（如 `portable-app.json`），让应用内置自更新保持禁用，由 Nix/`nix-update` 负责更新。
- **EasyCLIProxyAPI 与 CLIProxyAPI 内核的对接**：EasyCLIProxyAPI 启动时按可写基准目录下 `cpa-core/cli-proxy-api`（内核可执行文件名固定为 `cli-proxy-api`）是否存在来判断“内核已安装”，并需要同目录的 `config.example.yaml` 模板来生成 `config.yaml`。仓库已有的 `cliproxyapi` 包（上游 Go 源码构建）产出的是 `bin/server`，但其 `src` 里有 `config.example.yaml`：在 wrapper 里 `ln -sfn ${cliproxyapi}/bin/server <runtime>/cpa-core/cli-proxy-api` 并 `install -C -m644 ${cliproxyapi.src}/config.example.yaml`，再写一个 `cpa-gui-meta.json`（`{"version":"v<ver>","assetName":"...","installedAtUnix":0}`）即可让 GUI 直接使用 Nix 内核，避免其内嵌/下载的上游二进制在 NixOS 上因缺少 `/lib64/ld-linux-x86-64.so.2` 而 `exit status 127`（`NixOS cannot run dynamically linked executables ...`）。内核按 `-config <path>` 传参，`server` 与 `cli-proxy-api` 参数兼容。
- **Vite 单文件静态站（vite-plugin-singlefile）**：此类 WebUI（如 Cli-Proxy-API-Management-Center）`bun run build`/`npm run build` 只产出 `dist/index.html`（资源内联）。用 `buildNpmPackage`（`bun.lock` 需先自行生成 `package-lock.json`，见 Lockfile 规则）并覆盖 `installPhase` 为 `mkdir -p $out; cp -r dist/. $out/`，不要走默认的 npm install hook（否则会装进 `$out/lib/node_modules`）；`env.VERSION = finalAttrs.version` 可让 `__APP_VERSION__` 注入正确版本（否则源码里 `git describe` 失败会退化成 `dev`）。单文件站通常还应额外产出 `management.html` 之类的服务端约定文件名（如 `ln -s index.html $out/management.html`），便于被后端直接以固定路径提供。
- **上游 release 直接提供成品静态资产时另建 `<pkg>-bin` 包**：若发布页每个 tag 都挂着固定文件名、不含版本号的成品资产（如 Cli-Proxy-API-Management-Center 的 `management.html`），不要一律用 `buildNpmPackage` 自行构建，而是另建一个 `-bin` 包：`fetchurl` 指向 release 资产（URL 用 `${finalAttrs.version}` 插值）+ `dontUnpack = true` + `installPhase` 里 `install -Dm644 $src $out/<原名>`（后端要求固定文件名时再 `ln -s` 补别名）+ `passthru.updateScript = nix-update-script { }` + `sourceProvenance = [ binaryNativeCode ]`。实测 nix-update 对固定文件名资产同样会重写 `version` 行并 prefetch 新 URL 的哈希（可用 `nix-update <attr> --version <旧版本> --flake` 往返验证后 `git checkout` 还原）。源码构建包与 `-bin` 版并存：前者服务需要可复现构建/打补丁的消费者，后者服务只消费成品（如 NixOS 模块的离线静态面板）的场景，两者接口保持一致（都提供 `management.html`）就不会牵动模块。

## NixOS 模块规范

- **自有模块放在 `modules/` 并在 `flake.nix` 的 `nixosModules` 注册**：本 fork 的自有模块（`pgy`/`cpolar`/`hkdm`/`aw88399-legion-audio`/`cliproxyapi`）集中放在 `flake.nix` 的 `# My additions` 段；模块会被 `flake-modules/_internal/modules-test-nixos-config.nix` 自动编入测试用的 `nixosConfigurations`，因此 `enable` 默认 false 时也必须能正常求值（用 `pkgs.<pkg> or null` 之类的可选默认值，避免在未应用 overlay 时求值失败）。
- **自有模块选项命名空间统一用 `dataEraserc` 前缀**：为避免与未来可能加入的 nixpkgs 官方模块冲突，所有 fork 自有模块的选项路径统一加 `dataEraserc` 中间层：`services.dataEraserc.<name>`（如 `services.dataEraserc.cliproxyapi`、`services.dataEraserc.pgy`）或 `hardware.dataEraserc.<name>`（如 `hardware.dataEraserc.aw88399-legion-audio`）。`flake.nix` 的 `nixosModules` 注册名保持不变（不加前缀），因为 `builtins.attrValues self.nixosModules` 要求平铺结构。
- **与 nixpkgs 已有同名选项冲突时用 `disabledModules` 接管**：nixpkgs 已自带 `services.cliproxyapi` 模块，直接声明同名选项会报 `already declared`。要自定义行为（离线面板、可写配置、自动密钥等）时，在自有模块顶层写 `disabledModules = [ "services/misc/cliproxyapi.nix" ]`（路径相对 `modulesPath`，即 `nixos/modules/`），再完整声明选项与 `config`。注意这会全局替换 nixpkgs 的模块，凡是导入本模块的 NixOS 都受影响。
- **服务内置从 GitHub 下载的静态面板时，用 store 路径 + 关闭自动更新实现离线**：CLIProxyAPI 的 `/management.html` 由 `managementasset.FilePath` 解析，优先读环境变量 `MANAGEMENT_STATIC_PATH`（目录则取 `<dir>/management.html`），文件不存在才联网。让模块传 `MANAGEMENT_STATIC_PATH = <runCommand 面板目录>` 并在默认设置里加 `remote-management.disable-auto-update-panel = true`，即可离线且不写只读 store。`runCommand` 里用构建期 `[ -e <pkg>/management.html ]` 判断，兼容只提供 `index.html` 的面板包。
- **运行期密钥优先用 `._secret`，不要用会改变行为的 `MANAGEMENT_PASSWORD`**：CLIProxyAPI 支持 `MANAGEMENT_PASSWORD` 环境变量，但它会强制 `allow-remote=true`（绕过 `remote-management.allow-remote`）。NixOS 模块里应复用 `utils.genJqSecretsReplacement { loadCredential = true; }`，把 `managementKeyFile`（默认自动生成到 `${stateDir}/management.key`）通过 `remote-management.secret-key._secret` 注入，这样 `allow-remote` 仍生效，密钥也不进 store。
- **可写配置文件用「首启 seed 一次」开关**：GUI/管理 API 会写回配置文件，若模块每次 activation/启动都用 Nix 值覆盖会丢失运行期改动。做法是把 nixpkgs 风格的 secret 替换脚本（`utils.genJqSecretsReplacement ... .script`）包在 `preStart = if mutableConfig then ''if [ ! -e ${configPath} ]; then ${script}; fi'' else script;` 里：默认 seed-once（保留面板改动），设 `mutableConfig = false` 则每次声明式重写。
- **seed-once 的配置文件必须让 secret 轮换显式失效**：`config.yaml` 里存的是已解析的密钥值，轮换密钥后 seed-once 会让**旧密钥永远留在盘上**（服务继续接受旧密钥）。做法：把 credential 内容摘要（`sha256sum $CREDENTIALS_DIRECTORY/<tag> | sha256sum`，tag 取 `secretsReplacement.credentials` 里 `"<tag>:<path>"` 冒号前的部分，注意**不能**用 `lib.escapeShellArg` 包 `$CREDENTIALS_DIRECTORY`，否则单引号会阻止运行时展开）与求值期的 settings 指纹（`builtins.hashString "sha256" (builtins.toJSON settings)`）一起写进 `${stateDir}/.config-fingerprint`：credential 变化时先 `rm -f config.yaml` 让 seed 重新生效，settings 变化只 `echo ... >&2` 警告（直接重写会丢面板改动），并保留 `if [ ! -e config.yaml ]` 的 seed 包裹。写 marker 前先 `rm -f` 再 `printf >`，避免旧所有者导致 `chmod 0600` 在 `set -e` 下失败。
- **应用启动时 `os.Chdir` 到可执行文件目录会让 `WorkingDirectory=` 彻底失效**：Go 程序常用 `os.Executable()` + `os.Chdir(filepath.Dir(exe))` 定位自身资源（m365-copilot2api 的 `cmd/server/main.go`），此时 systemd 的 `WorkingDirectory=` 毫无作用，程序的工作目录永远是只读的 store 子目录。凡是"优先读 `*_DATA_DIR`、否则退回相对路径/tmpdir"的变量（会话/对话缓存、debug 日志、API keys 等）只设 `*_DATA_DIR` 覆盖不全，写入会以 `open ./xxx.tmp: read-only file system` **静默失败**（接口照常 200，只丢数据）。模块必须把每个持久化路径用绝对路径环境变量显式指向 `stateDir`（对照上游 Dockerfile 的 env 清单，再加上 Dockerfile 也漏掉的那些相对路径项：如 `M365_CONVERSATION_CACHE` 默认 `conversations.json`、`M365_SESSION_CACHE` 默认 `sessions.json`、`M365_DEBUG_LOG` 默认 `debug-logs.jsonl`、`M365_API_KEYS` 默认 `$HOME/.config/...`）。验证方式：`journalctl -u <unit> | grep "read-only file system"`，以及检查 `stateDir` 里是否真的出现了这些文件。**复查日志必须按新实例的启动时间过滤**：先 `systemctl show -u <unit> -p ExecMainStartTimestamp` 拿时间戳，再 `journalctl -u <unit> --since <该时间>`，否则 `--since` 窗口会把旧实例（旧 PID/旧 syslog identifier）的报错一起抓进来，把"已修好"误判成"仍有问题"——本次实测 `--since` 只多覆盖 29 秒就多出 14 条旧实例报错。
- **secret 轮换要感知"落盘优先"的上游逻辑，优先用 `*_BOOTSTRAP_FILE` 而不是复制明文**：这类网关常在首次读到明文密码后哈希写入 `admin-password.json`，之后启动只认该文件（`M365_DATA_DIR` 的优先级还高于 `M365_ADMIN_PASSWORD_FILE`），于是模块 `ExecStartPre` 每次覆盖的明文文件再也不会被读取——轮换 secret 后控制台密码静默保持旧值、旧密码仍然有效。正确做法：用上游的一次性 bootstrap 变量（`M365_ADMIN_PASSWORD_BOOTSTRAP_FILE`，只在持久化文件不存在时生效）+ systemd credential（systemd 会把 credential chown 给服务用户、模式 0400，**服务进程自己就能读**，不需要 `+` 的 ExecStartPre 把明文 `install` 进 `stateDir`），再用一个 sha256 标记文件（`${stateDir}/.<name>.sha256`）比对 credential 内容：内容变化时 `rm -f` 掉持久化文件让 bootstrap 重新生效，内容不变则保留（管理员在控制台改的密码不会被每次启动冲掉）。标记文件方案要在临时目录里用假 credential 跑一遍完整状态机（首次/未变/轮换/空文件/文件缺失）再上线；从旧版本升级时首次运行也要顺带删除遗留的明文副本。
- **`Environment=` 不做 specifier / 变量展开，credential 路径要用 wrapper 或固定路径传递**：实测 `Environment=FOO=%d/bar` 会原样保留 `%d`，`$CREDENTIALS_DIRECTORY` 同样不展开，所以不能靠 `Environment=` 把 credential 的真实路径喂给应用。可行做法：(1) `ExecStart` 指向 `pkgs.writeShellScript` 包装脚本，在脚本里读 `$CREDENTIALS_DIRECTORY/<name>` 后 `export` 应用需要的变量（也能 `export FOO="$(cat ...)"` 传 secret 而不经 systemd 解析，避免 `EnvironmentFile` 的转义坑）；(2) 直接用固定路径 `/run/credentials/<unit-name>/<name>`。注意 `Environment=` 里写的 secret 会出现在 `systemctl show` 输出里，wrapper 里 export 的不会。
- **不要声明应用根本不读的选项**：加"预留参数"类选项（如 `extraArgs`）前先确认上游有没有命令行解析——`grep -rn 'flag\.\|os.Args' cmd/ api/` 无结果就说明参数永远不会生效（m365-copilot2api 的 server 只读环境变量）。同理，模块的 `stateDir` 若非 `/var/lib/<name>` 就不能靠 `StateDirectory=` 创建，此时 `ReadWritePaths=` 引用一个不存在的路径会让 `ProtectSystem=strict` 的 unit 启动失败，应在文档里声明该目录必须预先存在（`builtins.dirOf stateDir == "/var/lib"` 时才设 `StateDirectory`）。
- **所有可写目录都要被 `StateDirectory`/`ReadWritePaths` 覆盖，断言必须能被求值**：把 `stateDir`、`authDir` 等可写目录分成两组——`builtins.dirOf dir == "/var/lib"` 的进 `StateDirectory`（多个目录直接传列表，systemd 会逐个创建并 chown 给 `User`），其余进 `ReadWritePaths`，activation 里再 `install -d -m 0700` + `chown` 兜底，否则 `ProtectSystem=strict` 下"目录存在但不可写"或"路径不存在导致 unit 启动失败"。（实测旧版 `cliproxyapi` 模块只把 `stateDir` 放进 `ReadWritePaths`，`authDir` 设为 `/var/lib/cpa-auth` 时该路径完全没被覆盖。）断言 message 必须写实际命名空间（`services.dataEraserc.<name>.*`），自定义 `user`/`group` 用 `builtins.hasAttr cfg.user config.users.users` 校验存在性；`package = null` 时不能直接写 `lib.getExe cfg.package`（断言生效前就抛类型错误），要写成 `if cfg.package != null then lib.getExe cfg.package else "${pkgs.coreutils}/bin/false"`，让断言给出可读报错。
- **activation script 中的 `umask` 必须用子 shell 隔离**：NixOS 所有 activation snippets 在同一 bash 进程中顺序执行，一个 snippet 里的 `umask 077` 会泄漏到后续所有 snippet（尤其是 `etc` snippet，它调用 `setup-etc.pl` 创建 `/etc` 符号链接树）。如果 `setup-etc.pl` 在 `umask 077` 下运行，`make_path` 创建的目录权限会是 `0700` 而非 `0755`，导致非 root 用户无法读取 `/etc/systemd/user.conf` 等关键文件，所有 systemd user manager（包括 GDM greeter）都会报 `Permission denied` 而无法启动桌面。修复方法：`umask 077` 必须包在子 shell 里，例如 `(umask 077; openssl rand -base64 32 > keyfile)`，确保 umask 变更不逃逸。注意 `if` 块不会创建子 shell，所以 `umask` 在 `if` 内外都有效。
...
- **依赖哈希必须暴露为顶层 passthru 属性，否则 nix-update 不会更新**：nix-update 从顶层包的属性读取依赖哈希（`pkg.pnpmDeps.outputHash`、`pkg.goModules.outputHash`、`pkg.npmDeps.outputHash` 等，见 nix-update 的 `eval.nix`/`dependency_hashes.py`），只更新它看得见的。像 axonhub 那样把 `frontendPnpmDeps = fetchPnpmDeps { ... }` 放在 let 里、由中间派生 frontendDist 消费的模式，nix-update 完全看不到，版本升级后 pnpmDeps 哈希永远静默过期。修复：`passthru.pnpmDeps = frontendPnpmDeps;`（it-tools 直接把 pnpmDeps 写在顶层 mkDerivation 上所以没这个问题；resin 同理用 `passthru.npmDeps = frontendDist.npmDeps;`）。同时注意 nix-update 的哈希更新顺序里 pnpmDeps 在 goModules 之前：若 pnpmDeps 过期，goModules 预取构建会先因依赖（frontend-pnpm-deps）失败而中止，`extract_hash_from_nix_error` 从报错 stderr 里抓**最后一个** `got:` 哈希写入 vendorHash——实测它会把 pnpm store 的哈希写进 vendorHash 字段（axonhub 曾因此 vendorHash 变成 pnpm 哈希）。暴露 pnpmDeps 后顺序保证先修复 pnpmDeps 再预取 goModules，可避免此污染
- **固定输出派生的静默陈旧复用**：FOD（fetchPnpmDeps/goModules 等）的输出路径由 `name + 声明哈希` 决定、与派生其余输入无关。若声明哈希是旧 lockfile 时代算出来的，且旧 FOD 的输出还在本地/远端 builder 的 store 里，则换了新 src 的新派生会因输出路径同名同哈希而**直接复用旧输出、跳过重新下载**，不报哈希不匹配。症状：构建时报 `ERR_PNPM_NO_OFFLINE_TARBALL`（store 里缺少 lockfile 新增的包，且 store 里混着新旧两个版本的包索引）。修复：把哈希清空重建拿到真实哈希写回。这也意味着“构建成功”不代表 FOD 内容与当前 lockfile 一致
- **跟踪 git HEAD 且有依赖哈希的包一律用 nix-update-script --version branch，不用 unstableGitUpdater**：`unstableGitUpdater`（update-source-version）只处理 version/rev/src hash，不更新 vendorHash/cargoHash/npmDepsHash，上游依赖变了会静默过期（oidc-tester-app 的 vendorHash、resin 的 npmDepsHash 曾因此构建失败）。已把所有带依赖哈希的 unstableGitUpdater 用户迁移到 `nix-update-script { extraArgs = [ "--version" "branch" ]; }`（baidupcs-go、drone-file-secret、ldap-auth-proxy、pterodactyl-wings、fake-ollama、helium-gateway-rs、sidestore-vpn、linguaspark-server、hack3ric-flow）。branch 模式从分支 atom feed 取 HEAD 日期，并用 releases feed 的最新 tag 数字作版本前缀（无 tag 仓库自动得 `0-unstable-<日期>`，等效 hardcodeZeroVersion），同一次运行里更新 rev/src hash 并预取 goModules/cargoDeps 刷新依赖哈希。无依赖哈希的包（内核模块、字体、词典、纯 Python 包等）可继续用 unstableGitUpdater。注意事项：(1) atom feed 日期取 entry 的 `updated` 字段，与 unstableGitUpdater 的 committer date 可能相差一天（fake-ollama 实测出现过日期回退）；(2) 仓库 releases 混入无关 tag 时版本前缀会失真，需改用 version-regex 或 unstableGitUpdater 的 tagFormat/tagConverter；(3) 迁移前用 `git ls-remote --tags` 和 releases.atom 确认版本前缀格式不变
...
- **脚本必须幂等**：进入包管理器之前先取 `SRC_BEFORE=$(nix build --no-link --print-out-paths ".#$UPDATE_NIX_ATTR_PATH.src")`，`nix-update "$UPDATE_NIX_ATTR_PATH" --src-only` 之后再取一次，两者相同就 `exit 0`（源码没变 → lockfile 不可能有理由变化）。否则每次更新都会让 semver 范围整体浮动、重写 lockfile 与 `npmDepsHash`，产出"版本号不变、只改哈希"的空转提交并让二进制缓存反复失效（实测 cliproxyapi-management-center 连续 4 次提交只改 `npmDepsHash` 与 `@rolldown/binding-*`、`@oxc-project/types` 等锁定版本）。重新生成前还必须把已提交的 lockfile 复制进新源码树做种子（`cp "$SCRIPT_DIR/package-lock.json" package-lock.json`）再 `npm install`，否则已锁定的版本会无谓上浮；shebang 里的 `nodejs` 要与派生中的 `nodejs = nodejs_24` 对齐。
