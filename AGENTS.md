...
- **与 nixpkgs 已有同名选项冲突时用 `disabledModules` 接管**：nixpkgs 已自带 `services.cliproxyapi` 模块，直接声明同名选项会报 `already declared`。要自定义行为（离线面板、可写配置、自动密钥等）时，在自有模块顶层写 `disabledModules = [ "services/misc/cliproxyapi.nix" ]`（路径相对 `modulesPath`，即 `nixos/modules/`），再完整声明 `services.cliproxyapi.*` 选项与 `config`。注意这会全局替换 nixpkgs 的模块，凡是导入本模块的 NixOS 都受影响。
