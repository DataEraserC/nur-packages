...
- **包装类包不能丢掉上游 wrapper 的 `--add-flags`（Electron 的 Wayland 输入法尤其致命）**：`bqqnt` 这类“取上游 `qq` 的 `share/` + 自写启动脚本”的包装包，若脚本直接 `exec ${qq-original}/bin/qq`，就会丢掉上游 wrapper 注入的 flag——nixpkgs `qq` 在 `NIXOS_OZONE_WL` 时加 `--ozone-platform=wayland --enable-features=WaylandWindowDecorations --enable-wayland-ime=true --wayland-text-input-version=3`（缺了它 Wayland 下的 Electron 既不建 text-input 也不走 XIM，fcitx5 完全打不出中文；GNOME 只支持 text-input-v3，`--wayland-text-input-version=3` 必需），而 fork 内自有的 `pkgs/uncategorized/qq` 没有这组 flag，且自有包会经 `callPackageWith (pkgs // _packages // …)` 遮蔽 nixpkgs 同名包，`qq-original ? qq` 拿到的正是它。包装包要用 `makeWrapper --add-flags` 把 flag 补到真实启动路径上，并让**每个** desktop 入口都指向带 flag 的 wrapper：直接从上游 `share/` 拷来的 desktop 其 `Exec=` 是上游二进制的绝对 store 路径，不改就会绕过 wrapper（同包两个入口行为不一致）。转义要点：写 `--add-flags "\''${NIXOS_OZONE_WL:+\''${WAYLAND_DISPLAY:+…}}"`，只有 `\''${` 既躲开 Nix 插值又把 `$` 留给运行时 shell；只写 `''${` 会在构建期被 shell 展开成空串、flag 静默消失。验证时把 `qq-original` 换成打印 `"$@"`/`$LD_PRELOAD` 的 stub 后 `.override`，即可在无 GUI 环境断言 flag 与 `LD_PRELOAD` 都在。
