...
- **包装包优先依赖 nixpkgs 的上游包，不要在 fork 里再包一份旧副本**：`bqqnt` 曾经依赖 fork 自带的 `pkgs/uncategorized/qq`（3.2.18），而 nixpkgs 的 `qq`（3.2.32）wrapper 已经带了 `NIXOS_OZONE_WL`+`WAYLAND_DISPLAY` 条件下的 `--ozone-platform=wayland --enable-features=WaylandWindowDecorations --enable-wayland-ime=true --wayland-text-input-version=3`；自写启动脚本 `exec ${qq}/bin/qq` 会把这些 flag 全丢掉，Wayland 下 Electron 既不建 text-input 也不走 XIM，fcitx5 完全打不出中文（GNOME 只支持 text-input-v3，`--wayland-text-input-version=3` 必需）。**注意 `createCallPackage` 用 `callPackageWith (pkgs // _packages // …)`，fork 内同名包会遮蔽 nixpkgs 的版本**（`qq ? qq` 拿到的是自有 3.2.18），要显式依赖 nixpkgs 的版本就把 `pkgs` 写进参数表（nixpkgs 的 `pkgs.pkgs` 存在，所以注入的合并集里能取到 `pkgs`）再用 `pkgs.qq`；这样版本与依赖跟着 flake.lock 走，本仓库不必再维护一份上游派生（`helpers/update.nix` 的 excludes 注释里说的“纯 wrapper 跟随 flake.lock”就是这类包，无 `passthru.updateScript` 即可）
- **只加一层的包装包用 `symlinkJoin`**：`symlinkJoin { paths = [ pkgs.qq ]; … }` 由 `lndir` 造出真目录 + symlink 的树，直接暴露上游的 `bin`/`share`（图标与上游 desktop 一起继承），postBuild 里只补自己新增的文件；不做 `cp -r`/`chmod -R`，share 永不与上游脱节。要点：(1) `makeWrapper` 不能写到"已存在且指向 store"的 symlink 上（`echo > $wrapper` 对只读 store 会 EACCES），要先 `rm -f`；(2) `sed -i` 会把 store symlink 换成真文件，不会改到 store 原件；(3) `symlinkJoin` 不继承 `meta`，`description`/`license`/`homepage` 要自己给（`meta = qq.meta // { … }`），此时 `meta.position` 落在自己的文件上，`tools/build_own_cachix.py` 会把它算作自有包（若继承上游 meta，position 指上游、会被该脚本的路径过滤掉）；(4) 上游 wrapper 用 `--prefix LD_PRELOAD` 追加自己的库（nixpkgs 的 qq 会插 libssh2），所以外层 `--set LD_PRELOAD=bstar` 不会被它清掉，两个库都会加载。验证时把 `qq` 换成打印 `"$@"`/`$LD_PRELOAD` 的 stub（`callPackage … { pkgs = pkgs // { qq = stub; }; }`）即可在无 GUI 环境断言注入与参数都在；若确实要在自己的 wrapper 里补这组 flag，条件写法是 `--add-flags "\''${NIXOS_OZONE_WL:+\''${WAYLAND_DISPLAY:+…}}"`（只有 `\''${` 同时躲开 Nix 插值并把 `$` 留给运行时 shell，只写 `''${` 会在构建期被展开成空串）
...
- **包含 xddxdd 或 DataEraserC**：维护者列表必须非空，且至少包含仓库负责人之一（xddxdd 或 DataEraserC）
- **自有目录统一引用共享维护者列表**：nixpkgs 没有 `DataEraserC` 维护者条目，`with lib.maintainers; [ DataEraserC ]` 求值期就报 `undefined variable`（`nix-update` 读 `meta.maintainers`、`tools/check_package_meta.py` 都会触发）。`pkgs/uncategorized-DataEraserC/` 内的包一律写 `maintainers = import ../maintainers.nix;`（该文件在迁移保留目录内，改一处即全目录生效；条目必须含 `name` 与 `github`——前者被 `nix-update` 取用，后者被 `check_package_meta.py` 取用），并且必须 `git add`，否则 flake 求值报 `Path 'pkgs/uncategorized-DataEraserC/maintainers.nix' ... is not tracked by Git`
...
`meta.maintainers` 的语义是“谁负责、能 ping 谁”（`commonMeta` 还会把 `meta.teams[*].members` 并进 maintainers），AI 承担不了该责任，因此**AI 一律不写进 maintainers**（`pkgs/uncategorized-DataEraserC/` 继续只 `import ../maintainers.nix`）；AI 的参与改用下面两个渠道记录：
...
- **包级来源用 `passthru.aiProvenance`**（机器可读、不进 `meta`、不影响构建）：
  ...
  字段语义：`agent`（必填，agent 运行时/工具，如 `dsh`、`opencode`）、`model`（自己确实在用的模型；**无法确证时省略，不要猜**）、`involvement`（`authored` 从零编写 / `assisted` 参与修改 / `reviewed` 仅审阅）。查询：`nix eval --json .#<pkg>.passthru.aiProvenance`
- **变更级来源用 commit trailer**：`passthru` 覆盖不到的文件（`modules/`、`update.sh`、AGENTS.md 等）只能靠它，AI 实质参与的提交加一行
  ...
  格式 `<agent>:<model>`，沿用 Linux 内核 AI Coding Assistants 的 `Assisted-by` 约定；不要用 `Co-authored-by`（把模型写成团队成员会扰乱 `git blame` 的责任归属）
- **不要新增自定义 `meta.*` 字段**：`meta` 是受校验的 record，`config.checkMeta = true`（nixpkgs CI 的 `ci/eval/outpaths.nix` 正是如此）下会硬失败 `key 'aiProvenance' is unrecognized`，而默认 `checkMeta = false` 让本地毫无感觉——等于埋雷；`passthru` 不在校验范围内
- **只在实质性参与时标注**：编写/修改构建逻辑、模块、更新脚本才标（纯机械批量改动如统一 maintainers 不标，历史无法重建就留空——provenance 失真比缺失更糟）；`passthru` 只写在迁移时保留的 `pkgs/uncategorized-DataEraserC/` 目录内，`pkgs/uncategorized/` 会在下一轮重建时被覆盖，那里的参与只能靠 commit trailer
