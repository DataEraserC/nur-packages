...
- **为修改版 Firefox 关闭 PGO**：`buildMozillaMach` 默认在 x86_64-linux 上启用 PGO（`pgoSupport = true`），其 `profilingPhase` 会用 `./mach python ./build/pgo/profileserver.py` 启动一次 Firefox 采集 profile。但重度补丁版的 Firefox（如 `feder-cr/firefox_antidetect_patch` 的反指纹分支）会破坏内容进程的干净关闭：`Quitter.quit()` 触发的关闭过程中内容进程 hang，被进程监视器 SIGABRT，LLVM profile runtime 来不及 flush，导致 `llvm-profdata merge` 报 `truncated profile data` / `no profile can be merged`，整个 `profilingPhase` 失败（上游只跑 `./mach build`、从不测试 PGO）。修复方法：用 `.override { pgoSupport = false; }` 关闭 PGO（LTO 仍由 `ltoSupport` 默认开启，保留）。注意 `buildMozillaMach` 的 `pgoSupport` 在其内部派生函数的参数里、不在外层 opts 里，因此不能直接写进 `buildMozillaMach { ... }`，而要用派生的 `.override` 传：`((buildMozillaMach { ... }).override { pgoSupport = false; }).overrideAttrs (old: { ... })`。
  - **PGO 在 ESR 153 + 反指纹 rebase 补丁上可正常启用**：把补丁集从 feder-cr 150/151 源码 rebase 到原版 Firefox 153.0esr（本文档另述的 3-way merge）后，上述 PGO 崩溃不再复现——`profilingPhase` 干净跑完（约 11 分钟），`llvm-profdata` 生成 `/build/merged.profdata` 成功，第二轮用 `--enable-profile-use=cross --with-pgo-profile-path=/build/merged.profdata -fprofile-use=/build/merged.profdata` 重编成功产出可运行的 `firefox`（`Mozilla Firefox 153.0esr`）。因此 `lantianCustomized.firefox-unwrapped` 现在保留 `pgoSupport = true`（直接不传 `.override { pgoSupport = false; }`），并把 `maxSilent` 提到 21600（6h）以容纳 PGO+LTO 全量构建（远程 EPYC 上 libxul 的 cross-LTO 单次链接 ~50min、~53GB RSS，是整个构建的最长极点）。若未来上游 ESR 版本再出现 PGO profilingPhase hang，再回退 `.override { pgoSupport = false; }`。
...
- **从丢失的补丁源码反向提取 patch**：当上游只提供一个已打补丁的源码 tarball（git 仓库已丢失/不可用），需要把它转换为可在原版源码上应用的 patch 集时：下载对应版本的原版 release tarball（`https://archive.mozilla.org/pub/firefox/releases/<ver>/source/firefox-<ver>.source.tar.xz`），用 `diff -rq` 列出所有差异，再用 git 反向生成 `diff --git` 格式补丁。流程：在原版源码目录 `git init && git add -A && git commit`，然后把补丁版里**真正属于源码改动**的文件覆盖/新增进去（跳过仓库脚手架如 `.mozconfig`、`.github/workflows`、`.hg*`、`scripts/`、`README.md` 前言、`.gitignore`、`.gitattributes`，以及 release 产物 `sourcestamp.txt`、`__pycache__/`、`*.git_archival.txt` 等），`git add -A` 后用 `git diff --cached -- <pathspec>` 按子系统分组导出多个 `.patch`。**二进制资源不能进文本 patch**（如 `browser/fonts/*.ttf` 共 219MB），应把对应的 `moz.build` 列表与文本清单（`bundle-fonts.list`）放进 patch，二进制本体由独立 derivation 在 `postPatch` 阶段复制到位。生成后务必在全新解压的原版 tarball 上用 `patch -p1 --dry-run`（nixpkgs 用 `patch`）和 `git apply --check` 双重验证，再用 `cmp` 逐文件比对应用结果与补丁源码一致。
- **跨 Firefox 版本 rebase patch 集需 3-way merge + 人工移植 API 漂移**：把基于 N 版的补丁集迁到 M 版（如 151→153esr）时，逐文件 `git merge-file <M-orig> <N-orig> <N-patched>` 做三方合并（base=N-orig、ours=M-orig、theirs=N-patched），可自动吸收纯行号偏移的 hunk；冲突文件手动解（一般取“M 的新增 + theirs 的补丁逻辑”叠加）。但**三方合并只能处理行级变更，检测不到“补丁新增代码用了旧 API”的语义冲突**：典型如 153 把 `gfxFontEntry* Xxx::CreateFontEntry(...)` 改成 `already_AddRefed<gfxFontEntry> ...`，补丁里 stealth 分支新增的 `auto* fe = new CTFontEntry(...); ...; return fe;` 会被静默保留为裸指针返回，导致编译失败。这类必须手动移植：`new T(...)`→`MakeRefPtr<T>(...)` / `MakeAndAddRef<T>(...)`，裸 `return fe;`→`return fe.forget();`，`delete fe;`→直接 `return nullptr;`（RefPtr 自动释放）。rebase 完用 `patch -p1 --dry-run` 验证 0 fuzz/0 fail，并 grep `return new .*FontEntry(` 与 `auto\* .* = new .*FontEntry` 复查所有平台 font-list 的 stealth 分支。另外注意 nixpkgs `buildMozillaMach` 按版本号挂默认补丁（如 `136-no-buildconfig.patch`、`133-env-var-for-system-dir.patch`），它们会先于我们的补丁应用；rebase 时要确认这些补丁不触碰我们改的文件，且自家旧的兼容补丁（如 `153-cbindgen-0.29.4-compat.patch`）若已上游修复合入则在新版上变成 reversed/no-op，必须从 `patches` 列表中删除。
- **mozbuild `FINAL_TARGET_FILES` 列表必须按 lowercased 文件名排序**：`StrictOrderingOnAppendList.ensure_sorted` 用 `sorted(l, key=lambda x: _first_element(x).lower())` 校验，未排序会抛 `mozbuild.util.UnsortedError: We expected "arial.ttf" but got "Candara.ttf"`。动态生成文件名列表时**不要用 `LC_ALL=C sort -f`**：它把字母 fold 成大写后比较，会改变下划线 `_` 与字母的相对顺序（`l_10646.ttf` vs `LeelaUIb.ttf`：`sort -f` 给出错误顺序，而 Python `sorted(key=str.lower)` 正确），与 mozbuild 不一致。用 python 生成：`python3 -c 'import glob;fs=sorted(glob.glob("*.ttf")+glob.glob("*.ttc")+["bundle-fonts.list"],key=str.lower);print(",".join(chr(34)+f+chr(34) for f in fs),end="")'`。本仓库 `lantianCustomized.firefox-unwrapped` 的 `moz.build` 用占位符 `"@@STEALTHFONTS@@"`，`postPatch` 把 windows-fonts 的字体拷进 `browser/fonts/`、用 `gen_bundle_font_manifest.py`（需 `fontTools`）重新生成 `bundle-fonts.list`（74 个 Microsoft 家族，自动过滤掉 TwemojiMozilla），再把上述 python 排序结果 substituteInPlace 进 `moz.build`。手改 patch hunk 后务必重新生成（`git diff --no-index` 或 `diff -u --label`），否则 `@@ ... @@` 行号计数会过期，`patch` 会静默丢掉最后一行（出现过 `SyntaxError: '[' was never closed`）。
...
- **打包二进制（加壳）的等长补丁 + cd 包装**：对加壳/自解密且无 section headers 的专有二进制（如 PgyVisitor 的 `pgyvisitor`），若 store 路径过长无法替换硬编码的 `/usr/share/pgyvpn`（17 字节），可用**等长字节补丁**把字符串原位替换为相对路径并用 `///` 补齐到相同长度（如 `/usr/share/pgyvpn` → `./share/pgyvpn///`）。补丁必须保持字节数完全一致（用逐字节 `substr` 覆盖而非 sed/perl -0pi 整体替换），否则变长替换会改变后续字节偏移、导致加壳解包逻辑崩溃（SIGSEGV，地址形如 `0x408f...`）。注意不能用 `\0` 补齐——NUL 会截断后面拼接的 `/res/custom.json` 等子串，导致 `Is a directory` 错误；应用 `///` 这类空路径段补齐（POSIX 路径中连续 `/` 会折叠）。随后把真实二进制移到 `$out/libexec`，用 `makeWrapper ... --chdir $out` 包装到 `$out/bin`，使相对路径 `./share/pgyvpn` 解析到 store 内资源。
- **硬编码路径守护进程的 NixOS module**：对这类专有 daemon（如 `pgyvpn_svr`），module 应**基于 FHS 包**（`pgy_fhs`）而不是在系统层建符号链接。`buildFHSEnv` 的 rootfs 内部已把 `/usr/bin`、`/usr/sbin`、`/usr/share`、`/etc`（只读）映射好，module 里不再需要任何 `/usr`、`/etc` 符号链接。**读写数据与配置用 bwrap 绑定实现**：nixpkgs 新版 `buildFHSEnv` 支持 `extraBwrapArgs`，可在其中追加 `--bind stateDir /etc/oray/pgyvpn`（数据/状态读写绑定，位于 rootfs 只读绑定之后、叠加生效）与 `--bind configFile /etc/oray/pgyvpn/config.ini`（配置文件绑定，**一律用 rw `--bind`，不要用 `--ro-bind`**——守护进程会改写配置文件；store 路径（如 `writeText`）因 store 只读而天然只读，store 外的绝对路径（`externalPath`，如 `/etc/xxx/config.ini` 或 out-of-store symlink 目标）则可读写并同步回原文件）。包内暴露 `dataDir`/`configFile` 两个可选参数映射到 `extraBwrapArgs`，module 用 `package.override { dataDir = ...; configFile = ...; }` 注入。**configFile 的 option 类型用 `lib.types.oneOf [ lib.types.pathInStore lib.types.externalPath ]` 区分只读 store 文件与可读写外部路径，无需手写 enum**（注意 `lib.types.externalPath` 为 nixpkgs 26.05+ 新增，需基于 unstable/26.05+ 分支才能使用）。要点：`ExecStart` 通过 FHS 包装器运行守护进程（`${pkg}/bin/pgy pgyvpn_svr ...`，用字符串列表传参），`-f` 指向绑定的 `/etc/oray/pgyvpn/config.ini`；守护进程默认前台运行（不要传 daemonize 参数），systemd 用 `Type = "simple"` + `Restart = "on-failure"`；激活脚本只需 `mkdir -p` `stateDir`/`logDir`（`/var` 在沙盒里读写绑定）。注意 FHS 顶层输出没有 `etc/`（配置在 rootfs 里），不能引用 `${pkg}/etc/...`。模块引用包遵循仓库约定（`package` 默认 `null`，`enable` 时 assertion 强制设置）。**activation script 的依赖名不能随便用**：新版 nixpkgs（unstable / 26.05+）已移除默认的 `activationScripts.system`，若用 `lib.stringAfter [ "system" ]` 会报 `attribute 'system' missing`（在 `strings-with-deps.nix` 的 `predefined.${entry}` 处）。应依赖稳定存在的默认脚本，如 `lib.stringAfter [ "specialfs" ]`（默认脚本有：`specialfs`、`stdio`、`var`、`usrbinenv`、`groups`、`users`、`etc`、`modprobe`、`udevd` 等）。**非 oneshot 服务的 ExecStart 不能是列表**：NixOS 会把 `serviceConfig.ExecStart` 的列表值序列化成多行 `ExecStart=` 设置，而 systemd 260+ 对非 `Type = "oneshot"` 服务拒绝这种写法（`Service has more than one ExecStart= setting`）。应改用 `lib.escapeShellArgs (列表)` 生成单个 `ExecStart` 字符串（它会自动按需加引号，safe 于含空格的参数）。**守护进程运行时会写回配置的 configFile 不能默认只读 store 文件**：若登录/运行会向 `config.ini` 写入状态（如账号信息），默认值不应是 `pkgs.writeText`（store 只读，绑定 `--bind` 后仍是只读，运行时报写入失败）。应默认一个可写的外部路径（如 `"${cfg.stateDir}/config.ini"`），并在激活脚本中先 `[ ! -e ]` 检查、不存在则用 `cat > ... <<'EOF'` 写入默认配置（注意 heredoc 的 `EOF` 定界符必须顶格写在 Nix `''...''` 字符串的第 0 列，nixfmt-rs 会重新缩进 `''` 内容但 Nix 会按收尾 `''` 的缩进剥除前缀，语义仍正确；`externalPath` 类型要求值是纯字符串而非 path 字面量，且检查 store 前缀区分只读/可写）。**NixOS 服务应提供 `user`/`group` 选项**：守护进程服务默认以 root 运行（VPN 类需要 root 建虚拟网卡/路由），但应暴露 `user`/`group` 两个 `lib.types.str` 选项（默认 `"root"`）并设置到 `serviceConfig`，激活脚本对 `stateDir`/`logDir`/`configFile` 执行 `chown -R user:group`（store 路径等只读文件用 `|| true` 容忍失败），以便用户选择以专用用户上下文运行提升安全性。

- **上游会随版本更新丢弃旧补丁的 patch 型源码（如 `aw88399-legion-audio` 系）**：不要 vendored 静态快照（`lib.cleanSource ./source` + 固定 `version`），应改用一个每周自动拉上游并把全部历史补丁留存归档的镜像仓库（配套仓库 `Apps-Used-By-Myself/16iax10h-linux-sound-saga`，其 `patch-archive/*.patch` 永久包含每个内核标签的最终版 + 历史修订）。nvfetcher 段用 `fetch.git` + `src.git` 跟踪镜像默认分支，段名加 `AAA_` 前缀防 toml-sort 打乱顺序；包内 `inherit (sources.AAA_xxx) version src;`，`installPhase` 从解包根目录按相对路径拷贝（补丁包拷 `patch-archive/*.patch`，同仓库固件包拷 `fix/firmware/...`）。由于纯 flake 求值无法枚举 fetchgit 内容（需 IFD），模块里"可选版本"仍是**硬编码列表**；把 `patchVersion` 从 `enum` 改为 `nullOr str`（默认 `null` = 从列表挑"≤ 实际内核 `config.boot.kernelPackages.kernel.version` 且主次版本同系列"的最大版本），列表为空时 `throw`（覆盖过新/过老内核）。**直接读 `config.boot.kernelPackages.kernel.version` 来选版是安全的，不会与 `boot.kernelPatches` 构成求值递归**（已用 evalModules + 真实 `linuxPackages` 实证：kernel 派生的 `.version` 属性惰性求值，读取它不强制 `kernelPatches` 字段），不必退而用 `pkgs.linux` 默认内核当基准（那会让"用户选了非默认内核"的配置误选/误报）。自动选仅含正式标签、排除 `-rc*` 与 `.rev*`；`assertions` 那套"默认 vs 实际"比对不再需要。
