...
- **上游会随版本更新丢弃旧补丁的 patch 型源码（如 `aw88399-legion-audio` 系）**：不要 vendored 静态快照（`lib.cleanSource ./source` + 固定 `version`），应改用一个每周自动拉上游并把全部历史补丁留存归档的镜像仓库（配套仓库 `Apps-Used-By-Myself/16iax10h-linux-sound-saga`，其 `patch-archive/*.patch` 永久包含每个内核标签的最终版 + 历史修订）。nvfetcher 段用 `fetch.git` + `src.git` 跟踪镜像默认分支，段名加 `AAA_` 前缀防 toml-sort 打乱顺序；包内 `inherit (sources.AAA_xxx) version src;`，`installPhase` 从解包根目录按相对路径拷贝（补丁包拷 `patch-archive/*.patch`，同仓库固件包拷 `fix/firmware/...`）。由于纯 flake 求值无法枚举 fetchgit 内容（需 IFD），模块里"可选版本"仍是**硬编码列表**；把 `patchVersion` 从 `enum` 改为 `nullOr str`（默认 `null` = 从列表挑"≤ nixpkgs 默认内核 `pkgs.linux.version` 的最大版本"），并用 `assertions` 比对实际 `config.boot.kernelPackages.kernel.version` 与自动选中版本的主次版本号，不一致时报错提示显式设置 `patchVersion`。注意不能从 `config.boot.kernelPackages` 反推选择结果，否则会与 `boot.kernelPatches` 构成求值递归；自动选仅含正式标签、排除 `-rc*` 与 `.rev*`。
