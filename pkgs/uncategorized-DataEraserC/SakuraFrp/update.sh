...
FILE="$(dirname "$(readlink -f "$0")")/SakuraFrp.nix"
NEW_VERSION=$(curl -fsSL 'https://nya.globalslb.net/natfrp/client/launcher-unix/' |
  grep -oP '[0-9]+\.[0-9]+\.[0-9]+' | sort -V | tail -n1)
[ -n "$NEW_VERSION" ] || {
...
if old == new:
    print(f"SakuraFrp already at {new}")
    sys.exit(0)
...
block_re = re.compile(
    r'url = "(https://nya\.globalslb\.net/natfrp/client/launcher-unix/[^"]+)";\n(\s*)hash = "[^"]*";'
)
text, n = block_re.subn(repl, text)
open(path, "w").write(text)
print(f"SakuraFrp updated to {new} ({n} hashes)")
PYEOF
