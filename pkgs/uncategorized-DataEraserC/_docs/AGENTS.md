...
- **选 npm tarball 还是 GitHub tag 当 src，先解包把上游测试跑一遍**：发布包的 `files` 白名单常把测试依赖的文件漏在外面（dsh-notifier 的 tgz 带 `test/` 却不带 `examples/`，而 `test/consumer-demo.test.mjs` 在加载期就读 `examples/consumer-demo/package.json`），以 tgz 为 src 时 checkPhase 必挂一个（实测 1826/1827）；白名单覆盖不全就改用 `fetchFromGitHub` 的 tag（tag 的 package.json version 与 tag 一致时无需 postPatch sed）
...
- **上游回归门不在 `test` script 里时，checkPhase 直接跑它**：上游只有 `check`/`gate`/`e2e` 等自定义命令时，可跑其中离线的回归门（host/client 双语 harness、破坏性变体、loopback HTTP 投递这类全离线 gate 在沙箱内约 30 秒可跑完），而 `e2e`/`live` 才需要外网——`doCheck = true` + 自定义 `checkPhase` 跑前者即可；这类 gate 往往在模块加载期就 `import '@deepseek-ai/dsh-home-paths'`/`dsh-tools`，故 lockfile 必须解析 peerDependencies（见上条）
- **上游「先打 tag、再 bump package.json」的 DSH 插件**：tag `vX.Y.Z` 的 package.json 仍是上一版（release 资产 tgz 才是 bump 后 pack 的，但不含 `.sandbox/`），所以 src 取 fetchFromGitHub tag、postPatch 用 sed 对齐 version，且 `update.sh` 生成 lockfile 前要做同一条 sed（否则 lockfile 根 version 每次更新都变）
